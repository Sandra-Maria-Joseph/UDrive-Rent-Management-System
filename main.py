...
manager = { "Milind" : "milind"}


#connection to database
conn = pyodbc.connect('Driver={SQL Server Native Client 11.0};'
                      'Server=DESKTOP-2CK63EH\SQLEXPRESS;'
                      'Database=vehicle_rent_management_system;'
...
@cross_origin()
def Vehicles():
    data1 = pd.read_sql("SELECT * FROM vehicle_details", conn)
...
@cross_origin()
def Drivers():
    data2 = pd.read_sql("SELECT * FROM Driver", conn)
...
@cross_origin()
def Details():
    data4 = pd.read_sql("SELECT * FROM Billing", conn)
